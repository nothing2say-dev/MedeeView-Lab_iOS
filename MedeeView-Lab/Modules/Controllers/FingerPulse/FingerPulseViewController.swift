//
//  FingerPulseViewController.swift
//  MedeeView-Lab
//
...
class FingerPulseViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        thresholdLabel.text = "Place finger till the image turned red"
    }
...
extension FingerPulseViewController {
    fileprivate func handle(buffer: CMSampleBuffer) {
        ...
        if (hsv.1 > 0.5 && hsv.2 > 0.5) {
            DispatchQueue.main.async {
                self.thresholdLabel.text = "Hold finger ☝️ still"
                self.toggleTorch(status: true)
            ...
            pulseDetector.reset()
            DispatchQueue.main.async {
                self.thresholdLabel.text = "Place finger till the image turned red"
            }
        }
    }
}
